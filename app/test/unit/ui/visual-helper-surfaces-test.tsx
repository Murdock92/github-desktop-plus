...
import { renderBranchHasRemoteWarning, renderBranchNameExistsOnRemoteWarning } from '../../../src/ui/lib/branch-name-warnings'
...
describe('visual helper surfaces', () => {
  it('renders octicons and creates octicon wrapper elements', () => {
    ...
    const wrapper = createOcticonElement(octicons.check, 'wrapper-icon', 'icon-id')
  ...
  it('renders donut segments for non-zero values and preserves the aria label', () => {
    ...
    const view = render(<Donut valueMap={valueMap} ariaLabel="CI status summary" />)

    const donut = view.container.querySelector('svg.donut')
    const paths = Array.from(view.container.querySelectorAll('path')).map(path =>
  ...
  it('renders remote-tracking and duplicate-name branch warnings when applicable', () => {
    ...
    const warnings = Array.from(view.container.querySelectorAll('.warning-helper-text'))
